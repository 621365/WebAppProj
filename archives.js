...
function timewarp(time) {
  ...
  if (((time < 0) ? (time * -1) : time) > 2147483647) {
    ...
    new alert("Search Error", time.toString() + " is not within the acceptable time range.");
    return -1;
  } else {
    ...
    document.getElementById(datedArchive[kissie].meta.uuid).scrollIntoView({behavior: ((window.matchMedia("(prefers-reduced-motion: reduce)").matches)?'instant':'smooth'), block: 'center', inline: 'nearest'});
...
function determineUTC() {
  let uUTC = userUTC();
  new alert("UTC Dialation", "Your UTC Dialation in your location is " + uUTC.toString() + ".");

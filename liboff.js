...
self.addEventListener("fetch", async function(e) {
 ...
 caches.match(e.request, {"ignoreSearch": true}).then(async function(response) {
     ...
     if (new URL(e.request.url).hostname !== window.location.hostname) return fetch(e.request);
      ...
      try { // Try and not use cachebar
        ...
        try {
          if (response.url.slice(0, ("https://621365.github.io/WebAppProj/").length) == "https://621365.github.io/WebAppProj/") return response; else return (fetch(e.request));
        } catch (e) {
          // fix for offline?
          return (fetch(e.request) || fetch(response.url));
        }
        if (!(response instanceof Response)) return (fetch(e.request)); else if (result instanceof Response) return response; else return (fetch(e.request));
      }
      
      
      // Caching 3.0 (Scrapped due to errors)
      /*let result = await fetch(e.request).then((r) => { if (r.ok) {result = r; return r;} else { throw new Error(""); } }).catch(async function (e) {
            let tr = fetch(response.url).then((r) => { if (r.ok) {result = r; return r;} else {throw new Error("");}}).catch(async function (e) {
                ...
                if (result.ok) return result; 
                    ...
                    try {
                        if (response == undefined || !(response instanceof Response)) throw new Error(""); if (response.url.slice(0, ("https://621365.github.io/WebAppProj/").length) == "https://621365.github.io/WebAppProj/") return response; else return (fetch(e.request));
                      } catch (e) {
                        // fix for offline?
